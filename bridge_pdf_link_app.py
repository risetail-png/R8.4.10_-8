    ...
    try:
        ...
        for didx, plist in diag_to_photo_pages.items():
            ...
            for pp in plist:
                ...
                label = (f"→ {span_label}写真番号 {min(photo_nums)}〜{max(photo_nums)}"
                         if photo_nums else f"→ {span_label}損傷写真 p.{pp+1}")
        ...
        for pp, dlist in photo_to_diag_pages.items():
            ...
            for didx in dlist:
                ...
                span_label = f"{span}径間・" if (is_multi_span and span) else ""
                btn_list.append((f"← 損傷図（{span_label}{title}）", didx))
